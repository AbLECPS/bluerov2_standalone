...
class PlotReachability:
    ...
    def callback(self,result):
        try:
            ...
            self.times.append(self.count/1.0)
            self.min_distances.append(self.min_distance)
            self.count+=1
        except:
            pass
      
    def fls_callback(self,fls):
        try:
            self.min_distance = min(fls.data,60)
    ...
    def animate(self,i,commands,times):
        ...
        try:
            ...
            ax.set_title('Verification Result vs Distance to Obstacle (FLS)', color='#5B5B5B',loc='left',pad=25.0,fontweight="bold",fontsize=30)
