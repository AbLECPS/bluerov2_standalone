...
class TaskHandler(py_trees.behaviour.Behaviour):
    ...
    def __init__(self, 
                 ...
                 fdir_params="{'user_choice':'override_threshold','am_s_threshold':0.5, 'am_threshold':0.5}"
                ):                 
...
############<<USER INIT CODE BEGINS>>##############################
        ...
        if (not os.path.exists(fdir_path)):
            fdir_path = os.path.join(alc_config.WORKING_DIRECTORY, fdir_path)
...
############<<USER UPDATE CODE BEGINS>>##############################
        ...
        if (self.am and len(self.lec_input__msg.data[0:self.ann_input_len]) == self.ann_input_len):
            ...
            rospy.loginfo("[FDIR] %s class: %d (#%d, %0.2f), comb_am: %0.2f,\033[1;3%dm decision: %d\033[0m" % (
                uuv_state_str, prediction, degraded_id, efficiency, combined_am_output, 1+decisions["comb"], decisions["comb"]))
